...
#                 "playlist_url": f"https://open.spotify.com/playlist/{top50_playlist_id}",
...
# # Fetch Spotify audio features with safe defaults
...
#                 if f and f.get('id'):
#                     # Use safe defaults if Spotify returns None
...
#                         (f.get('tempo') if f.get('tempo') is not None else DEFAULT_FEATURES['tempo']) / 200.0
#                     ])
...
#         # Safe defaults for target vector
#         target_vec = np.array([
...
#         for t in tracks:
#             t_vec = features_map.get(t['id'])
#             t['similarity'] = cosine_similarity(t_vec, target_vec)
